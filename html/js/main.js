$(document).ready(function () {
    ...
    Application.prototype = {
        init: function () {
            ...
            // Init Data
            (function () {
                fetch(that.domainRoot + "json/statics.json")
                    .then((response) => {
                        if (!response.ok) {
                    ...
                    .then((data) => {
                        ...
                        that.pageFrom = data.todayItemsPageFrom;
                        that.pageEnd = data.todayItemsPageEnd;
                        that.pageCurrent = params.get("page");
                        if (that.pageCurrent < that.pageFrom || that.pageCurrent > that.pageEnd) {
                            that.pageCurrent = data.todayItemsPageFrom;
                        }
                        that.fetchData();
                    })
                    .catch((error) => {
        ...
        drawUI: function (data) {
            const that = this;
            const enArr = shuffleArray(data);
            const cnArr = shuffleArray(data);
        ...
        drawUIExample: function (item) {
            ...
            return function () {
                ...
                if (item.examples && item.examples.length > 0) {
                    ...
                    that.exampleElement.appendChild(nameDiv);
                    ...
                    for (var i = 0; i < item.meanings.length; i++) {
                        ...
                        for (var k = 0; k < item.meanings[i].length; k++) {
                            if (k === 0) {
                                ...
                                optionSpan.innerHTML = item.meanings[i][k].length > 10 ? '<span style="font-size:8px;">' + item.meanings[i][k] + "</span>" : item.meanings[i][k];
                    ...
                    that.exampleElement.appendChild(optionDiv);
                    ...
                    that.exampleElement.appendChild(sentenceDiv);
                    that.exampleElement.style.display = "block";
                    that.bindEventsExample();
                }
            };
        },
        bindEvents: function () {
