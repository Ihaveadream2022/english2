<template>
    <div>
        ...
        <div class="searchBox" ref="searchBox">
            <el-form :inline="true" :model="table.searchForm" ref="searchForm" class="demo-form-inline" label-width="100px" size="mini" @submit.native.prevent>
                ...
                <div class="searchBtnBox">
                    ...
                    <el-form-item>
                        <el-button type="primary" @click="readingGenerate">Generate JSON Files</el-button>
                    </el-form-item>
                </div>
            </el-form>
        </div>
        ...
        <!-- 查看器 -->
        <el-dialog :title="this.viewDialog.form.title" :close-on-click-modal="false" :close-on-press-escape="false" class="vehm" :visible.sync="viewDialog.visible" @close="viewDialogClose" @opened="viewDialogOpen" width="1200px" top="0" center>
...
<script>
    /* eslint-disable */
    import { readingList, readingAdd, readingEdit, readingDelete, readingGenerate } from "@/api/request";
        ...
        data() {
            return {
                ...
                editDialog: {
                    ...
                    visible: false,
                    rules: {
                        title: [{ required: true, message: "Please input reading title", trigger: "blur" }],
                    },
                },
                viewDialog: {
                    title: "View Reading",
        ...
        methods: {
            ...
            editDialogClose() {
                this.$nextTick(() => {
                    ...
                    this.editDialog.title = "Create Reading";
            ...
            editDialogEdit(row) {
                this.editDialog.title = "Edit Reading";
                this.editDialog.visible = true;
                ...
                this.$refs.editDialogForm.validate((valid) => {
                    if (valid) {
                        ...
                        if (this.editDialog.operation == "add") {
                            readingAdd(this.editDialog.form)
                                .then(
                                    (res) => {
                                        Message({
                                            message: `${res.message}`,
                            ...
                            readingEdit(this.editDialog.form)
                                .then(
                                    (res) => {
                                        Message({
                                            message: `${res.message}`,
            ...
            editStatus(row, value) {
                ...
                readingEdit({ ...row, status: value }).then(
                    (res) => {
                        console.log("res", res);
                        loading.close();
                        this.tableGetList(this.table.searchForm);
            ...
            tableGetList(searchForm) {
                readingList(searchForm.createTime ? { ...searchForm, createTime: `${searchForm.createTime[0]},${searchForm.createTime[1]}` } : { ...searchForm })
                    .then(
                        (res) => {
